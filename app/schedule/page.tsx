...
type ScheduleFilterType = "All" | "On Track" | "Off Track"
...
function getSessionType(session: ScheduleSession): "On Track" | "Off Track" {
  if ("type" in session && session.type === "off-track") {
    return "Off Track"
  }
...
export default function SchedulePage() {
  ...
  const [selectedType, setSelectedType] = useState<ScheduleFilterType>("All")
  ...
  const typeFilters: ScheduleFilterType[] = ["All", "On Track", "Off Track"]
  ...
  const filteredSessions = useMemo(() => {
    return scheduleSessions.filter((session, index) => {
      ...
      const matchesType =
        selectedType === "All" || sessionType === selectedType

      const matchesBookmark =
        !bookmarkedOnly || bookmarks.includes(sessionId)

      return matchesDay && matchesType && matchesBookmark
    })
  }, [selectedDay, selectedType, bookmarkedOnly, bookmarks])
    ...
    <main className="iphone-page text-white">
      <header className="iphone-header">
        ...
        <p className="iphone-subtitle">
          Filter by day, filter on track or off track, bookmark sessions, and
          get reminders 30, 15 and 5 minutes before they start.
        </p>
      </header>
      ...
      <section className="mb-5 space-y-3">
        ...
        <button
          ...
          className={`h-11 rounded-full px-5 font-black flex items-center gap-2 transition ${
            bookmarkedOnly
        ...
        {filteredSessions.length === 0 && (
          <section className="iphone-card">
            ...
            <p className="text-white/60 mt-2">
              Try another day, change the on track/off track filter, or turn off
              bookmarked only.
            </p>
          </section>
        )}
